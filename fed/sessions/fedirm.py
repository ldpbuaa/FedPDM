...
class FedIRM(SessionBase):
    def __init__(
        ...
        self.labeled_cm_list = {c:[] for c in range(self.num_clients)}
        self.avg_labeled_cm = torch.ones(self.num_classes, self.num_classes) / self.num_classes
        # history
        self.init_his()
    ...
    def _round(self, rounds, clients, next_clients, epochs, sup_attr):
        ...
        for c, rv in results.items():
            ...
            self.labeled_cm_list[c] = rv['labeled_cm']
        ...
        self.avg_labeled_cm = self.avg_conf_matrix()
    ...
    def async_train(self, states, epochs, sup_attr):
        ...
        for c, s in states.items():
            kw = {
                ...
                'avg_labeled_cm':self.avg_labeled_cm
            }
            kwargs.append(kw)
    ...
    def avg_conf_matrix(self, ):
        if self.mode == 'mix':
            all_mat = [m for m in self.labeled_cm_list.values()]
        else:
            all_mat = [m for c,m in self.labeled_cm_list.items() if self.sup_attr[c] ]
        avg_mat = torch.cat(all_mat, dim=0).mean(dim=0)
        return avg_mat
