...
class FedPDMProcess(FedAvgProcess):
    def __init__(
    ...
    def _unsup_local_train(self, client, state, rounds, epochs, uld_dist, cost_matrix):
        ...
        try:
            for epoch in range(epochs):
                sel_pl, pl_cor, lds, uld_dist, f1, sims = self._semi_epoch(dataloader, labeledloader,
    ...
    def _semi_epoch(self, dataloader, labeledloader, uld_dist, avg_losses,
        ...
        for s in range(steps):
            ...
            with torch.no_grad():
                logits_w = self.ori_model(images_w)
            if self.mode == 'mix':
        ...
        return sel_pl_stats, pl_cor_stats, ld_stats, uld_dist, f1, sims
